...
class SyncedCronClass {
    constructor() {
        ...
        this.options = {
            ...
            // NOTE: Unset to remove expiry but ensure you remove the index from mongo by hand.
     ...
     *
     * @param {Object} opts
     * @param {boolean} opts.log - Log job run details to console, default is true.
     * @param {boolean} opts.logger - Use your own logger.
     * @param {boolean} opts.collectionName - Name of collection to use for synchronisation and logging.
     * @param {boolean} opts.utc - Default to using localTime.
     * @param {boolean} opts.collectionTTL - TTL in seconds for history records in collection to expire.
     *
     */
    config(opts) {
     ...
     * Add a scheduled job.
     *
     * @param {Object} entry - The job entry object.
     * @param {string} entry.name - The unique name of the job. *Required*.
     * @param {Function} entry.schedule - A function that determines when to run the job, using a laterParser. *Required*.
     * @param {Function} entry.job - The code that the job will execute. *Required*.
     * @param {boolean} [entry.persist=true] - Whether the job should persist between sessions. Optional, defaults to true.
     *
     * @throws {Error} if the entry properties do not meet the expected types.
     */
    add(entry) {
    ...
    /**
     * Return next scheduled date of first matching entry or undefined.
    ...
    /**
     * Pause processing, but do not remove jobs so that the start method will restart existing jobs.
     */
    pause() {
        if (this.running) {
            Object.values(this._entries).forEach(entry => {
     ...
     */
    stop() {
        Object.entries(this._entries).forEach(([name]) => {
    ...
    _entryWrapper(entry) {
        return async (intendedAt) => {
            ...
            if (entry.persist) {
                ...
                // this job.
                try {
                    jobHistory._id = await this._collection.insertAsync(jobHistory);
                } catch (e) {
            ...
            try {
                ...
                if (entry.persist) {
                    await this._collection.updateAsync({_id: jobHistory._id}, {
                        $set: {
                            finishedAt: new Date(),
                            result: output,
                        },
    ...
    // The following function is lifted from the later.js package, however
    ...
    _laterSetInterval(fn, sched) {
        let t = SyncedCron._laterSetTimeout(scheduleTimeout, sched), done = false;
    ...
    _laterSetTimeout = function (fn, sched) {
        ...
        function scheduleTimeout() {
            ...
            // Don't schedule another occurrence if no more exist synced-cron#41.
            if (!next[0]) {
                return;
            }

            let diff = next[0].getTime() - now, intendedAt = next[0];
...
SyncedCron = new SyncedCronClass()